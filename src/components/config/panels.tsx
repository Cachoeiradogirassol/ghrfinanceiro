      ...
      await createFn({
        data: {
          ...
          enterprise_restriction:
            role === "master" || restriction === "none"
              ? null
              : (restriction as Enterprise["value"]),
        },
      });
            ...
            <Select value={role} onValueChange={(v) => setRole(v as "user" | "master")}>
              ...
              <SelectContent>
                <SelectItem value="user">
                  <span translate="no">Operador</span>
                </SelectItem>
                <SelectItem value="master">
                  <span translate="no">Master</span>
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
                  ...
                  <TableCell className="text-xs">
                    {u.role === "master" ? (
                      ...
                      <Badge variant="outline">
                        {ENTERPRISES.find((e) => e.value === u.enterprise_restriction)?.label ??
                          u.enterprise_restriction}
                      </Badge>
                    ) : (
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setEditing(u as AdminUser)}
                      >
                      ...
                      <Button
                        size="sm"
                        variant={u.banned_until ? "default" : "outline"}
                        onClick={async () => {
                          try {
                            await activeFn({ data: { user_id: u.id, active: !!u.banned_until } });
                            toast.success(
                              u.banned_until ? "Usuário reativado" : "Usuário desativado",
                            );
                            qc.invalidateQueries({ queryKey: ["admin-users"] });
                          } catch (e) {
                      ...
                      >
                        {u.banned_until ? "Reativar" : "Desativar"}
                      </Button>
                    </div>
                  </TableCell>
                </TableRow>
...
function EditUserDialog({
  ...
  onSave: (data: {
    ...
    role: "user" | "master";
    password?: string;
    enterprise_restriction: Enterprise["value"] | null;
  }) => Promise<void>;
}) {
  const [displayName, setDisplayName] = useState("");
  const [email, setEmail] = useState("");
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Master sempre tem acesso total — restrição é ignorada.
            </p>
          </div>
          <div>
      ...
      <Card className="p-5">
        ...
        <form
          onSubmit={async (e) => {
            ...
            try {
              await upsertFn({
                data: { ...form, initial_balance: Number(form.initial_balance), is_active: true },
              });
        ...
        >
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setForm({ ...form, enterprise: v as Enterprise["value"] })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="flex items-center gap-2">
            <Switch
              checked={form.master_only}
              onCheckedChange={(v) => setForm({ ...form, master_only: v })}
            />
            <Label>Restrita ao Master</Label>
          </div>
                  ...
                  try {
                    await upsertCC({
                      data: {
                        ...
                        code: cc.code,
                        name: e.target.value,
                        enterprise: cc.enterprise,
                        master_only: cc.master_only,
                        is_active: !inactive,
                      },
                    });
                    toast.success("Salvo");
                  ...
                  try {
                    await upsertCC({
                      data: {
                        ...
                        code: cc.code,
                        name: cc.name,
                        enterprise: v as Enterprise["value"],
                        master_only: cc.master_only,
                        is_active: !inactive,
                      },
                    });
            ...
            <div className="mt-4 space-y-2">
              {(accs.data ?? [])
                ...
                .map((a) => (
                  <div
                    key={a.id}
                    className={`flex items-center gap-2 ${a.is_active === false ? "opacity-60" : ""}`}
                  >
                    <Input
                      defaultValue={a.name}
                      onBlur={async (e) => {
                    ...
                    <Badge variant={a.kind === "revenue" ? "default" : "secondary"}>
                      {a.kind === "revenue" ? "Receita" : "Despesa"}
                    </Badge>
                    {a.is_active === false && <Badge variant="outline">Inativa</Badge>}
                    <Button
                      size="icon"
                      variant="ghost"
                      aria-label="Arquivar ou excluir"
                      onClick={async () => {
  ...
  return (
    <Card className="p-4 flex flex-wrap items-end gap-2">
      ...
      <div className="flex-1 min-w-[200px]">
        <Label className="text-xs">Novo Centro de Custo</Label>
        <Input value={name} onChange={(e) => setName(e.target.value)} />
      </div>
      <div className="w-44">
        <Label className="text-xs">Empreendimento</Label>
      ...
      <div className="flex items-center gap-2">
        <Switch checked={masterOnly} onCheckedChange={setMasterOnly} />
        <Label>Restrito Master</Label>
      </div>
      <Button
        size="sm"
        onClick={async () => {
          ...
          try {
            await upsertCC({
              data: { code, name, enterprise, master_only: masterOnly, is_active: true },
            });
    ...
    <div className="flex items-center gap-2 pt-2">
      ...
      <Button
        ...
        onClick={async () => {
          if (!name) return;
          try {
            await upsertAcc({ data: { cost_center_id: ccId, name, kind, is_active: true } });
